...
Поэтому в кач-ве обходного пути я топорно изменю дату локального времени через `sudo date MMDDhhmm[[CC]YY][.ss]` и создам файл из 6 символов в имени `touch -t [[CC]YY]MMDDhhmm[.ss] Alpaca`, на который будет ссылаться симлинк `ln -s Alpaca Chi`. Так же мы не можем у этого юзера выполнять sudo команды, т.к мы не числимся в sudo , и у нас нет для этого прав. Поэтому я приписал себя в sudoers в предыдущем разделе.
...
- ~~P.S: Можно было бы сделать `touch -t 199011141212 Alpaca` и отправить Альпаку в прошлое. <ins>В кач-ве пруфа что мы знаем про touch -t</ins>. Но я уже не стал настолько заморачиваться~~.
